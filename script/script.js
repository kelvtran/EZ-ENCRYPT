async function genAESKey(keysize) {
    ...
    document.getElementById("aes-key-input-encrypt").value = secretKeyHex;
}

async function aesEncrypt(){

    // Get the key from the input field
    let key = document.getElementById("aes-key-input-encrypt").value;
...
async function aesDecrypt(){
    ...
    let key = document.getElementById("aes-key-input-decrypt").value;
    // Validate the key input
    if (key.length != 32 && key.length != 64) {
        alert("Invalid key length");
        return;
    ...
    // Get the IV and encrypted data from the input field
    let ciphertext = document.getElementById("aes-ciphertext").value;
    // Validate the ciphertext input
    if (ciphertext.length < 32) {
        alert("Invalid ciphertext length");
        return;
    }
    // Convert the IV and encrypted data to bytes
    let ciphertextBuffer = hexToBuffer(ciphertext);
    // Convert the IV to a buffer
    let iv = ciphertextBuffer.slice(0, 16);
    // Convert the encrypted data to a buffer
    let encryptedData = ciphertextBuffer.slice(16);
    ...
    return window.crypto.subtle.decrypt(
        ...
        document.getElementById("aes-plaintext-decrypt").innerHTML = decryptedText;
    });
}
...
function hexToBuffer(hex){
    ...
    // need to be half the length of the string as each value in Uint8Array is 8 bits
